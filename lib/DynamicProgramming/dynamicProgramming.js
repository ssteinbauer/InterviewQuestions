const dynamicProgramming = (function() {
  ...
  return {
    ...
    // TODO: this can be solved with a sliding window
    // 
    longestSubstring: function(input) {
      ...
      for (let j = 0; j < input.length; j++) {
        ...
        if (seenValues.has(curVal)) {
          // i never moves backwards
          i = Math.max(seenValues.get(curVal), i);
        }

        maxLength = Math.max(maxLength, j - i + 1);
        // if we see curVal again then i may become the next value
        seenValues.set(curVal, j + 1);
      }
